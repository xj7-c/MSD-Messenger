...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      id="main-sidebar"
      className="w-full md:w-80 lg:w-96 border-r border-white/10 bg-slate-950/40 backdrop-blur-xl flex flex-col h-full shrink-0 select-none overflow-hidden z-10 shadow-[4px_0_24px_0_rgba(0,0,0,0.2)]"
    >
      {/* Top Search & Actions */}
      <div className="p-3 border-b border-white/10 space-y-2.5 bg-slate-950/20">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search chats, E2EE keys, or nodes..."
              className="w-full pl-9 pr-3 py-1.5 bg-slate-900/50 backdrop-blur-md text-xs text-slate-100 placeholder-slate-400 rounded-xl border border-white/10 focus:outline-none focus:border-cyan-400/60 focus:ring-1 focus:ring-cyan-400/30 transition-all font-sans"
            />
          </div>
        ...
        {/* Tab Filters */}
        <div className="flex items-center gap-1 bg-slate-900/40 backdrop-blur-md p-1 rounded-2xl border border-white/10 text-xs">
          <button
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-white/5 scrollbar-thin">
        {filteredChats.map((chat) => {
          ...
          const otherParticipant = chat.participants.find((p) => p.user.id !== 'user-me')?.user;
          ...
          return (
            <button
              ...
              className={`w-full p-3 text-left transition-all flex items-start gap-3 relative cursor-pointer ${
                isActive
                  ? 'bg-cyan-500/10 backdrop-blur-md border-l-4 border-cyan-400 shadow-[inset_0_0_20px_rgba(6,182,212,0.1)]'
                  : 'hover:bg-white/5'
              }`}
            >
                ...
                <span
                  ...
                  title={
                    chat.isGroup
                      ...
                      : `Status: ${otherParticipant?.presence || 'offline'}`
                  }
                />
              </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {chat.unreadCount > 0 ? (
                    ...
                    <span className="text-[9px] font-mono px-1.5 py-0.2 rounded-full bg-white/10 text-slate-300 border border-white/10 shrink-0">
                      {lastMsg.transportProtocol === 'Wi-Fi Direct' ? '⚡ Wi-Fi Direct' : lastMsg.transportProtocol === 'BLE Mesh' ? '📡 BLE' : '🔒 E2EE'}
                    </span>
                  ) : null}
                </div>
              </div>
      ...
      <div className="p-2.5 bg-slate-950/60 backdrop-blur-md border-t border-white/10 flex items-center justify-between text-[11px] text-slate-400 font-mono">
        <div className="flex items-center gap-1.5">
          <ShieldCheck className="w-3.5 h-3.5 text-cyan-400" />
          <span>Local Key Vault: 0x88F2...</span>
        </div>
        <span className="text-emerald-400">100% Zero-Knowledge</span>
      </div>
    </aside>
  );
};
